...
class AsyncTypesearch(BaseClient):
    ...
    @overload
    async def sources(self, *, timeout: Optional[float] = None, extra_headers: Optional[Mapping[str, str]] = None) -> Sources: ...

    @overload
    async def sources(
        self, *, domain: str, timeout: Optional[float] = None, extra_headers: Optional[Mapping[str, str]] = None
    ) -> Source: ...
    ...
    async def sources(
        ...
        *,
        ...
        return await self._get(sources_path(domain), model, timeout, extra_headers)
